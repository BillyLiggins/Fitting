...
void FillHist(string filename,TH2D* hDeltaE, TH2D * hNormE){
				...
				int main(){
								...
								for (double energy=0;energy<25;energy++){
												...
												if(slicedBi_normE->GetEntries()>200 && slicedPo_normE->GetEntries()>200  ){	
																...
																if (false){
																				...
																				checkerCanvas->SaveAs(("plotChecker/bi/NormE_between_"+SSTR(energy_real)+"0_to"+SSTR(energy_real+0.1)+".tex").c_str());
																				...
																				checkerCanvas_2->SaveAs(("plotChecker/po/NormE_between_"+SSTR(energy_real)+"0_to"+SSTR(energy_real+0.1)+".tex").c_str());
																}
								...
								RMSCan->SaveAs("deltaE_RMSVsEnergy.tex");
								...
								NormRMSCan->SaveAs("NormE_RMSVsEnergy.tex");
								...
								c1->SetGrid();
								double percent[n];
								double percent_error[n];
								for (int i = 0; i <n; i++) {
									percent[i]=(NormE_po_RMS[i]-NormE_bi_RMS[i])/NormE_bi_RMS[i];
									percent_error[i]= percent[i]*sqrt(pow((NormE_RMS_bi_error[i]/NormE_bi_RMS[i]),2)+pow((NormE_RMS_po_error[i]/NormE_po_RMS[i]),2));
									std::cout << percent[i]<<"+/-"<<percent_error[i] << std::endl;
								}
								TGraphErrors * graph =new TGraphErrors(n,E,percent,error_E,percent_error);
								graph->SetTitle("#frac{#sigma_{#alpha}-#sigma_{#beta}}{#sigma_{#beta}}");
